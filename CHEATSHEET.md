...
|------|------|
| `gd` | 定義へジャンプ |
| `grr` | 参照一覧（この変数/関数の使用箇所） |
| `gy` | 型定義へジャンプ |
| `gi` | 実装へジャンプ（interface → 実装） |
| `<C-s>` (挿入モード) | シグネチャヘルプ（引数の型表示） |
| `<leader>k` | ホバードキュメント |
| `<leader>ca` | コードアクション |
| `<leader>rn` | シンボルリネーム |
| `<leader>e` | 診断情報（フロート） |
| `<leader>ih` | インレイヒント表示トグル |
| `]d` / `[d` | 次/前の診断（エラー・警告）へジャンプ |

有効なLSPサーバー: `lua_ls`, `ts_ls`, `biome`

> `]d`/`[d` は Neovim 0.11 組み込みデフォルト。`grr` も 0.11 デフォルトだが明示的に設定。
